...
pub enum AniError {
    ...
    #[error("too many frames: {0} (max {MAX_ANI_FRAMES})")]
    TooManyFrames(u32),

    #[error("frame path length {0} exceeds maximum {MAX_ANI_PATH_LEN}")]
    PathTooLong(usize),
}

/// Maximum number of animation frames allowed in a single ANI file.
const MAX_ANI_FRAMES: u32 = 10_000;

/// Maximum path length for a single frame image file reference.
const MAX_ANI_PATH_LEN: usize = 4096;
